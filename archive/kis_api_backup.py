...
class KisAPI:
    ...
    def get_investor_trend(self, code: str, cache: dict) -> dict:
        ...
        try:
            ...
            # ── 당일 (items[0]) ──────────────────────────────
            d = items[0]
            foreign_today = safe_int(d.get("frgn_ntby_tr_pbmn", 0))  # 외국인 순매수금액
            orgn_today    = safe_int(d.get("orgn_ntby_tr_pbmn", 0))  # 기관 순매수금액
            prsn_today    = safe_int(d.get("prsn_ntby_tr_pbmn", 0))  # 개인 순매수금액

            # 외국인 매수비율 (매수량 / (매수+매도))
            frgn_buy  = safe_int(d.get("frgn_shnu_vol", 0))
            frgn_sell = safe_int(d.get("frgn_seln_vol", 0))
            frgn_total = frgn_buy + frgn_sell
            foreign_ratio = round(frgn_buy / frgn_total * 100, 1) if frgn_total > 0 else 50.0

            # 기관 매수비율
            orgn_buy  = safe_int(d.get("orgn_shnu_vol", 0))
            orgn_sell = safe_int(d.get("orgn_seln_vol", 0))
            ...
            frgn_pbmn = safe_int(d.get("frgn_shnu_tr_pbmn", 0)) + safe_int(d.get("frgn_seln_tr_pbmn", 0))
            orgn_pbmn = safe_int(d.get("orgn_shnu_tr_pbmn", 0)) + safe_int(d.get("orgn_seln_tr_pbmn", 0))
            ...
            # ── 5일 누적 ─────────────────────────────────────
            frgn_5d = sum(safe_int(x.get("frgn_ntby_tr_pbmn")) for x in items[:5])
            orgn_5d = sum(safe_int(x.get("orgn_ntby_tr_pbmn")) for x in items[:5])
