...
def get_stock_name(code: str) -> str:
    ...
    try:
        ...
        # 💡 코드가 확실히 포함되어 있는 kr_theme_stocks 테이블을 검색합니다!
        row = conn.execute("""
        ...
        if row:
            # '가비아KOSDAQ079940' 같은 문자열에서 한글 이름만 깔끔하게 추출
            pure_name = re.sub(r'(KOSPI|KOSDAQ).*|\d{6}', '', row[0]).strip()
            return pure_name
    except Exception as e:
        pass
    
    return code  # DB에서 못 찾으면 그냥 코드 반환
        ...
        while True:
            try:
                ...
                for code, pos in self.positions.items():
                    ...
                    rate  = (curr - pos["entry_price"]) / max(pos["entry_price"], 1) * 100
                    
                    # 💡 여기서 6자리 코드를 한글 이름으로 변환!
                    stock_name = get_stock_name(code)
                    
                    print(f"   💼 {stock_name}({code}) [{pos.get('grade','실계좌')}] "
                          f"{rate:+.1f}% | 손절:{pos.get('stop_price', 0):,.0f} 목표:{pos.get('tgt_price', 0):,.0f}")
