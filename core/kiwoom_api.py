...
class KiwoomAPI:
    ...
    async def get_condition_codes(self, use_keywords: list = None,
        ...
        # ★ 2026-09-04: 재진단 — 키움 조건검색은 "조건별 1분당 1회" 제한이
        #   있는데(공식 문서), 재시도 대기가 3초뿐이라 같은 조건을 쿨다운
        #   위반하며 재요청하고 있었음. 최근 7일 로그 실측: 총 120회 조건
        #   조회 중 32회(27%) 완전실패(3회 재시도 소진, +0개), 그중 15건은
        #   서버가 능동적으로 웹소켓을 끊는 패턴(received 1000 Bye)으로
        #   레이트리밋 거부에 가까워 보임 — 단순 서버혼잡보다 우리 쪽
