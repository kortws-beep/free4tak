...
이 엔진은 core/sbot_strategy.py(실전 코드)의 SwingStrategy를 직접
import해서 그대로 재사용한다(calc_atr_levels/check_sell 등) — 손절/
목표/트레일링 배수를 여기서 따로 하드코딩하지 않으므로, 실전 파라미터가
바뀌면 이 백테스터도 자동으로 같이 바뀐다. 현재(2026-08-16) 실전 규칙은
ATR 추세추종 v3: 손절 = 매수가-ATR×2, 목표1 = 매수가+ATR×3(상한+20%),
목표1 달성 후 트레일링(고점-ATR×1.5). MA20이탈/보유기한(20영업일)
청산은 둘 다 실전에서 제거됐고(각 2026-06-23/07-06), 이 엔진도 함께
비활성 상태(SWING_MAX_HOLD_DAYS=9999 기본값) — 자세한 내용은
core/sbot_strategy.py 상단 docstring 참고.

과거(v2 이전) 이 파일은 nbot과 비교하는 표에 sbot을 "+8%/+15%/+25%
익절, -7% 고정손절, 11영업일 보유제한"으로 설명했었는데 이미 실전과
안 맞는 옛날 방식이었음 — ATR 기반으로 바뀌면서 통째로 폐기.
...
# ★ 2026-07-07: 실전 sbot이 기간매도(보유기한 강제청산) 로직을 완전히 제거하고
#   ATR only로 운영 중이라(사용자 결정), 백테스트 기본값도 이를 반영해 사실상
#   비활성화(9999영업일 — 도달 불가능한 값)로 변경. 과거 룰을 다시 실험해보고
#   싶으면 run_sbot_backtest.py의 --max-hold-days로 원하는 값을 지정하면 된다.
SWING_MAX_HOLD_DAYS  = 9999     # 최대 보유 영업일 (기본 비활성화 — 실전과 동일)
SWING_FORCE_SELL_RATE = 0.02   # 장기보유 강제청산 기준 (수익률 +2% 이하) — 비활성화 상태에선 미사용
...
class SBotBacktestEngine:
    ...
    # ----------------------------------------------------------
    # 장기 보유 청산 (config.max_hold_days 초과 + 수익 +2% 이하)
    # ★ 기본값은 SWING_MAX_HOLD_DAYS=9999로 사실상 비활성화(실전과 동일,
    #   2026-07-06 제거) — --max-hold-days로 명시적으로 지정할 때만 작동
    # ----------------------------------------------------------
