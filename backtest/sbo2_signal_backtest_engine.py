...
[매도 로직] sbo2.py _check_sell()과 동일한 ATR 단계별 피라미딩 재현:
  ...
  - 목표1(ATR×2.0, 상한 +20%) 도달 → 50% 매도, 손절↑(entry+ATR×1.0), 목표2=curr+ATR×3
    (★ 2026-08-17: 목표1만 3.0→2.0, 목표2 이후 재상향은 그대로 ATR×3.0 —
    사용자 지적을 kr_theme_finance.db 두 구간에서 비교 후 채택, 상세 사유는
    TRADE_ATR_TARGET_MULT 상수 옆 주석 참고)
...
# ★ 2026-08-17: 3.0→2.0 (실전 목표가1 배수 하향과 동기화, 상세 사유는
#   lina_bot/sbo2.py의 _check_buy() 동일 지점 주석 참고). 목표1 달성
#   *이후* 재설정 목표(stage>=1)는 이 상수를 안 쓰고 여전히 하드코딩된
#   ATR×3.0(아래 run() 내부, 실전 _check_sell()과 동일) — 초기 목표만
#   낮추고 재설정 로직은 그대로 두는 게 이번 변경의 핵심.
TRADE_ATR_TARGET_MULT = 2.0
