...
# 종목 풀 설정
POOL_SIZE         = 20
MIN_TRADE_AMT_B   = 5_000_000_000   # 24시간 거래대금 50억 (원)
MAX_CHANGE_RATE   = 28.0            # 상한가 근접 (+28% 이상) 제외
...
# 매수/매도 금액 (30만원 시드 기준)
BUY_1ST_AMT       = 100_000     # 1차 매수 10만원
BUY_2ND_AMT       = 50_000      # 2차 매수 5만원 (물타기)
...
STOP_LOSS_NIGHT     = -0.03   # 야간 (00~06시)
...
MAX_DAILY_LOSS   = 2            # 당일 손절 최대 2회

NIGHT_START = 0
NIGHT_END   = 6

LOOP_SLEEP  = 300               # 5분마다 루프
...
class CBot:
    ...
    def get_current_price(self, markets: list) -> dict:
        ...
        try:
            ...
            return {item["market"]: safe_float(item["trade_price"]) for item in res}
        except Exception as e:
            print(f"❌ 현재가 조회 오류: {e}")
            return {}
