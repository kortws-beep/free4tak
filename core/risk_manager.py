...
class RiskManager:
    ...
    def calc_buy_amount(self, score: int,
        ...
        [계산 순서]
        1. 점수 기반 기본 배수
        2. ★ 켈리 공식 보정 (과거 성과 기반 — master_trades.db)
        3. ATR 변동성 보정
        4. 테마 보정
        5. 자금 한도 적용
        ...
        kelly_factor = 1.0
        try:
            # ★ master_trades.db 우선 탐색 (절대경로 → 폴백 순)
            _base = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
            db_candidates = [
                os.path.join(_base, "master_trades.db"),  # 절대경로 최우선
                db_path,                                   # 호출자 지정
                os.path.join(os.getcwd(), "master_trades.db"),
                "master_trades.db",
                "trade_history.db",                        # 구 DB 폴백
            ]
            conn = None
            ...
            if conn:
                # ★ 테이블명 자동 감지 (master_trades vs trades)
                tables = [r[0] for r in conn.execute(
                    "SELECT name FROM sqlite_master WHERE type='table'"
                ).fetchall()]
                tbl = "master_trades" if "master_trades" in tables else "trades"

                # 종목별 최근 20건
                rows = conn.execute(f"""
                    SELECT profit_rate FROM {tbl}
        ...
        multiplier = max(multiplier, 0.6)  # 켈리 과도 보정 방지 — 최소 0.6배
