...
from backteste.strategy_eod import EodStrategy
...
class EBot:
    ...
    def _select_best(self, codes: list) -> tuple:
        """
        조건검색 결과 중 백테스트 점수가 가장 높은 1종목 선정.
        """
        from backteste.strategy_eod import EodStrategy
        from backteste.fetch_eod_data import save_targets
        
        # 1. 오늘 검색식에 걸린 모든 종목 일단 DB에 저장 (데이터 축적용)
        # targets 형식을 맞추기 위해 간단한 리스트 생성
        raw_targets = []
        for code in codes[:30]:
            ...
            if data:
                raw_targets.append({
                    ...
                    'volume': safe_float(data.get("acml_vol", 0)),
                    'value': safe_int(data.get("acml_tr_pbmn", 0)) // 100_000_000
                })
        save_targets(raw_targets)
        ...
        for t in raw_targets:
            code = t['code']
            # [마스터 지시] 기존 물리 필터 무시 (0원인 경우만 제외)
            if t['price'] <= 0: continue
            
            # 백테스트 엔진에게 물어보기
            res = engine.get_backtest_score(code)
            bt_score = res['total']
            
            candidates.append((code, bt_score, t))
            print(f"  🔍 분석: {code}({t['name']}) | 백테스트: {bt_score}점 | 사유: {res['msg']}")

        if not candidates:
            return None, None

        # 3. 백테스트 점수가 가장 높은 순으로 정렬
        candidates.sort(key=lambda x: x[1], reverse=True)
        
        best_code, best_score, best_data = candidates[0]
        
        # 최소 60점은 넘어야 매수 (너무 근거 없는 종목 방지)
        if best_score < 55:
            print(f"  ⚠️ 선정 실패: 가장 높은 점수가 {best_score}점으로 기준치(60) 미달")
            return None, None

        print(f"  🎯 최종 선정: {best_code}({self._name(best_code)}) | 점수: {best_score}점")
            ...
            try:
                ...
                _write_status({
                    ...
                    "positions_detail": {
                        code: {
                            "name":        pos["stock_name"],
                            "current_price": int(pos.get("current_price", 0)), # 여기에 추가!
