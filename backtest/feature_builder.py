...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    upper_wick = high - np.maximum(close, out["open"])
    lower_wick = np.minimum(close, out["open"]) - low
...
def get_market_data_at(loader: DataLoader,
    ...
    if df.empty or date not in df.index:
        return None
    row = df.loc[date]
    if hasattr(row, "shape") and len(row.shape) == 2: row = row.iloc[-1]
    try:
        price_map = {
        ...
        print(f"get_market_data_at 예외: {type(_e).__name__}: {_e}")
        return None
    return {
