...
async def cmd_help(ctx):
    # ★ 2026-07-10: execute_command() 라우터 기준으로 재정리 — 기존엔
    #   "단타봇"(nbot, 폐기됨)으로 잘못 표기돼 있었고(실제로는 !상태/!정지/
    #   !시작/!점수기준/!분석이 전부 sbot), sbo2 명령어는 아예 빠져있었고,
    #   !긴급중단/!긴급재개는 실제 명령어(!리스크중단/!리스크재개)와 이름이
    #   달랐고, !git 계열은 함수만 있고 라우터에 연결이 안 돼 있어 실제로는
    #   작동하지 않았음 — 전부 실제 코드에 맞게 수정.
    msg = """🦊 **키키 — 명령어 안 외워도 돼요!**
그냥 말하면 알아서 해줍니다 😄
...
**📊 성과/분석**
  `!성과`          — 오늘 손익
  `!전체성과 [일수]` — 전체 봇 합산 성과 (기본 30일)
  `!성과상세 [일수]` — 샤프/MDD/시간대별 (기본 30일)
  `!분석오늘`      — 오늘 매매 AI 분석
  `!분석이번주`    — 이번주 패턴 분석
  `!분석 코드`     — 특정 종목 분석
