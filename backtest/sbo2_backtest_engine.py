...
[사용법]
  from sbot_backtest_engine import SBo2BacktestEngine, SBo2BacktestConfig
  cfg = SBo2BacktestConfig(...)
  engine = SBo2BacktestEngine(cfg, db_path)
  engine.run()
...
# ★ 2026-07-07: 실전 sbo2도 기간매도(25일 보유기한 강제청산) 로직을 완전히
#   제거하고 ATR only로 운영 중이라(사용자 결정), 백테스트도 이를 반영해
#   사실상 비활성화(9999영업일 — 도달 불가능한 값)로 변경.
...
class SBo2BacktestEngine:
    ...
    def __init__(self, config: SBo2BacktestConfig, db_path: str):
        self.config       = config
