...
from backtest_engine_attribution import AttributionBacktestEngine as BacktestEngine, BacktestConfig
...
def get_scenarios(base: BacktestConfig) -> list:
    """기본 비교 시나리오 — 손실 최소화 / 수익 극대화 가설 검증"""
    return [
        {
            "name": "기본(임계치60)",
            "config": {**base.__dict__, "buy_score_min": 60},
        },
        {
            "name": "보수적(임계치70)",
            "config": {**base.__dict__, "buy_score_min": 70},
        },
        {
            "name": "공격적(임계치55)",
            "config": {**base.__dict__, "buy_score_min": 55},
        },
        {
            "name": "포지션多(max=8)",
        ...
        {
            "name": "포지션少+보수적(max=3,임계치70)",
            "config": {**base.__dict__,
                       "buy_score_min": 70,
                       "max_positions": 3},
        },
        {
            "name": "포지션少(max=3,임계치60)",
            "config": {**base.__dict__,
                       "buy_score_min": 60,
                       "max_positions": 3},
        },
        {
            "name": "실AI점수(cache+임계치70)",
            "config": {**base.__dict__,
                       "buy_score_min": 60,
                       "max_positions": 3,
                       "ai_score_mode": "cache"},
...
def run_one(name: str, config: BacktestConfig, db_path: str) -> dict:
    ...
    engine.save_attribution(
        attr_report,
        os.path.join(os.path.dirname(db_path), "..", "results",
                     f"attribution_{safe_name}.json"),
    )
