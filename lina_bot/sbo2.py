...
- 시드머니   : 600만원 / 1종목 기본 100만원 / 최대 6종목 (2026-09-21: 150만×4→100만×6 재구성)
- 매수금액   : 점수 비례 (BASE_BUY_AMT 기준 ±조정)
...
# ★ 2026-09-21: 150만×4 → 100만×6으로 재구성(대장 결정) — 백테스트
#   비교 결과 절대수익률은 150만×4가 조금 더 높지만(+155% vs +141%),
#   MDD가 거의 절반(-24%→-13%)으로 줄고 샤프/칼마 등 위험대비수익은
#   뚜렷이 개선(거래횟수도 111→178건으로 자본회전 활발). "슬롯은
#   비었는데 살 게 없다"던 09-21 문제도 슬롯 여유가 늘어 완화될 것으로
#   기대. 대장: "수익 늘어나면 6으로 놔두고 매입금액을 올려가면 되니까"
#   — 슬롯수(6)는 고정, 나중엔 BASE_BUY_AMT만 올리는 방향으로.
...
#   정규장(15:20) 마감 이후 계속 열어두도록 변경 — 매도체크(08:00~20:00)와
#   동일하게 09:20~20:00은 매수/매도 둘 다 가능.
BUY_END_TIME     = "2000"       # 매수 마감
...
class Sbo2:
    ...
    def run(self):
        ...
        #   sbo2 실제 설정값 그대로(목표1 ATR×2.0 — 08-17 튜닝 결과,
        #   sbot의 ATR×3와 다름). 최대 종목수는 BASE_BUY_AMT/MAX_POSITIONS
        #   상수 참조(2026-09-21 기준 최대 6종목).
        _notify(
            f"🚀 [영암9 SWING2] 스윙봇 가동\n"
        ...
        while True:
            try:
                ...
                #   수단이 전혀 없어, 재발급 전까지 모든 API가 조용히
                #   실패함. sbot과 동일하게 주말/휴장/장외 continue보다
                #   먼저 매 루프 체크하도록 추가(2026-09-07 새벽 sbot/sbo2
                #   동시 chk-holiday 실패 사고로 발견).
