...
SELL_3RD_RATE   = 0.15    # 3차 익절: +15% → 전량
...
class Strategy:
    ...
    def get_rule_score(self, data: dict) -> int:
        ...
        try:
            ...
            # ── ★ MACD 히스토그램 (+0~8) ─────────────────────
            if   macd_hist > 0:  score += 8    # 상승 모멘텀
            elif macd_hist < 0:  score -= 6    # 하락 모멘텀
            ...
            # ── ★ 스토캐스틱 (+0~6) ──────────────────────────
            if   stoch_k < 20:        score += 6   # 과매도 반등
            elif stoch_k > 80:        score -= 6   # 과매수 주의
            elif 40 < stoch_k < 60:   score += 2   # 중립
    ...
    def check_sell(self, code: str, pos: dict, now_t: str,
        ...
        buy2_allowed = (
            buy2_rate > 0
            or (buy2_rate < 0 and not (is_weak and BUY_2ND_WEAK_ONLY))
        )
        if (not buy2_done and stage == 0
            ...
            print(f"➕ 2차 매수 {code} | {buy2_rate:+.2%} {tag}")
        ...
        # ----------------------------------------------------------
        # ② 종가 매도 (15:15 이후) — ★ 개선: 더 적극적 청산
        # ----------------------------------------------------------
        if now_t >= EOD_SELL_TIME:
            is_theme = buy_tags.get(code) == "theme_buy"
            if is_theme:
                # 테마주는 +이상이면 매도, 음수면 다음날 보유 (테마 모멘텀 노림)
                if rate >= 0:
                    on_sell(code, qty, f"테마종가매도({rate:+.2%})", current)
                    peak_tracker.pop(code, None)
                    return "테마종가매도"
            else:
                # 일반 종목: 단계 무관, -3% 이상이면 강제 청산 (다음날 갭하락 회피)
                if stage >= 1:
                    on_sell(code, qty, f"종가매도({rate:+.2%})", current)
                    peak_tracker.pop(code, None)
                    return "종가매도"
                elif rate >= -0.03:
                    # ★ 개선: -3%까지는 종가매도 (기존 -1%~+1% → -3%~∞)
                    on_sell(code, qty, f"종가매도({rate:+.2%})", current)
                    peak_tracker.pop(code, None)
                    return "종가매도"
                else:
                    # -3% 미만은 손절선까지 기다림 (어차피 곧 손절)
                    pass
