...
from sbo2_backtest_engine import SBo2BacktestEngine, SBo2BacktestConfig
...
# sbot 시나리오 정의
# ============================================================
def get_sbot_scenarios(base: SBo2BacktestConfig) -> list:
    """
    스윙봇 비교 시나리오.
...
# 단일 실행
# ============================================================
def run_one(name: str, config: SBo2BacktestConfig, db_path: str) -> dict:
    print(f"\n{'=' * 60}")
    print(f"▶ [SBO2] {name}")
    print(f"{'=' * 60}")

    engine = SBo2BacktestEngine(config, db_path)
    engine.run()
...
def main():
    ...
    print(f"📋 [SBO2] 대상 종목 {len(codes)}개")
    if not codes:
        print("❌ 종목 없음 — fetch_history_fdr.py 먼저 실행하세요")
        sys.exit(1)

    # 기본 설정
    base_config = SBo2BacktestConfig(
    ...
    if args.compare:
        ...
        results   = []
        for sc in scenarios:
            cfg = SBo2BacktestConfig(**sc["config"])
